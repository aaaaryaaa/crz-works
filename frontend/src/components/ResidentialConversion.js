...
export default function ResidentialConversion() {
    ...
    const func=async()=>{
        const response=await axios.post('/api/form/adminformsnull',{a});
    ...
    return (
        <div className='text-black'>
            <h1 className='w-full m-5 text-3xl font-bold'>Residential Conversion</h1>
            <div className="flex h-screen">
                <div className="w-1/4 bg-gray-200 shadow-md p-4">
                    ...
                    <button
                        className={`w-full py-2 mb-2 rounded ${filter === 'accepted' ? 'bg-blue-500 text-white' : 'bg-gray-300'}`}
                    ...
                    <ul>
                        {filteredApplications.map(app => (
                            <li key={app.form_id} className="bg-white p-4 mb-2 rounded shadow ">
                                <div className='p-5 bg-gray-100 cursor-pointer hover:bg-yellow-200' onClick={()=>navigate('/adminviewappl',{state:{app,return1:'resiConversion',}})}>
