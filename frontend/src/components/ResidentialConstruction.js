...
export default function ResidentialConstruction() {
    ...
    const func=async()=>{
        const response=await axios.post('/api/form/adminformsacc',{a});
                    ...
                    <ul>
                        {filteredApplications.map(app => (
                            <li key={app.form_id} className="bg-white p-4 mb-2 rounded shadow">
                                <div className='p-5 bg-gray-100 cursor-pointer' onClick={()=>navigate('/adminviewappl',{state:{app,return1:'resiConstruction'}})}>
